...
class PercolationStats:
    ...
    def run(self, num_sims: int):
        """ Run a percolation simulation num_sims times and save the results """
        results = []
    ...
    def print_summary(self):
        ...
        print(f'95% Confidence Interval: ({(self.mean - 1.96*self.stddev):.5} - {(self.mean + 1.96*self.stddev):.5})')


def main():
    sim = PercolationStats(100)
    sim.run(100)
    sim.print_summary()
